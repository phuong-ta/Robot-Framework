...
class MorseDecoderLibrary(object):
    ...
    def speed_should_be_with_range(self, expected_speed):
        self._decoder.write(bytes('WPM\n', 'utf-8'))
        text1 = self._decoder.readline().strip().decode('utf-8')
        speed = int(text1[7:9])
        if speed <= int(expected_speed)*0.1 or  speed >= int(expected_speed)/0.1:
            raise AssertionError('Expected: ' + str(expected_speed) + ' got: '  + str(speed) + ' line: ' + text1)
    
    def speed_should_be_with_whatever_speed(self, expected_speed):
        self._decoder.write(bytes('WPM\n', 'utf-8'))
        text1 = self._decoder.readline().strip().decode('utf-8')
        speed = int(text1[7:9])
        if speed <= 0:
